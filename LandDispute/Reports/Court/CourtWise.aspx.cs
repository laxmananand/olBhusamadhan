...
public partial class LandDispute_Reports_Court_ThanaWise : System.Web.UI.Page
{
    ...
    private void bindRange()
    {
        ...
        DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType });
        if (dt.Rows.Count > 0)
        {
    ...
    protected void bindDistrict()
    {
        ...
        DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _CommissionaryCode, Rangeid });
        if (dt.Rows.Count > 0)
        {
    ...
    protected void bindSubDivision()
    {
        ...
        DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _DistCode });
        if (dt.Rows.Count > 0)
        {
    ...
    protected void bindCircle()
    {
        ...
        DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _SubDivision });
        if (dt.Rows.Count > 0)
        {
    ...
    protected void bindThana()
    {
        ...
        DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _Blockcode });
        if (dt.Rows.Count > 0)
        {
    ...
    protected void bindCour()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ddlCout.DataSource = dt;
            ddlCout.DataTextField = "courtName";
            ddlCout.DataValueField = "id";
            ddlCout.DataBind();
            ddlCout.Items.Insert(0, new ListItem("All", "0"));
        }
        else
        {
            ddlCout.DataSource = null;
            ddlCout.DataTextField = "name";
            ddlCout.DataValueField = "id";
            ddlCout.DataBind();
            ddlCout.Items.Insert(0, new ListItem("All", "0"));
        }
    ...
    protected void bindCourt_Grid()
    {
        ...
        SqlParameter _DistCode = new SqlParameter("@DISTRICTCODE", ddldistrict.SelectedValue == "0" ? "0" : ddldistrict.SelectedValue);
            ...
            SqlParameter _court_id = new SqlParameter("@courtID", ddlCout.SelectedValue == "0" ? "0" : ddlCout.SelectedValue);         
            DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _CommissionaryCode, _DistCode, _SubDivision, _Blockcode, _pscode, _court_id, GetRange_Code });
            if (dt.Rows.Count > 0)
            {
                Court_grid.Columns[6].FooterText = "Total :";
                Court_grid.Columns[7].FooterText = dt.AsEnumerable().Select(x => x.Field<int>("total")).Sum().ToString();
                Court_grid.DataSource = dt;
                Court_grid.DataBind();
        }
        else
        {
            Court_grid.DataSource = null;
            Court_grid.DataBind();
        }
        Panelnyaayaalay.Visible = false;
    ...
    protected void btnExpToExl_Click(object sender, EventArgs e)
    {
        ...
        {         
            ...
            else if (Panelnyaayaalay.Visible == true)
            {
                ExportExcel(grvnyaayaalay, Panelnyaayaalay);
            }
            else if (pnlgrid.Visible == true)
            {
                ExportExcel(Court_grid, pnlgrid);
            }
        }
        catch (Exception)
        {
    ...
    protected void lnkTotal_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ViewState["courtdiscode"] = linkbtn.CommandArgument.ToString().Trim().Split(',')[0];
            ViewState["courtsubcode"] = linkbtn.CommandArgument.ToString().Trim().Split(',')[1];
            ViewState["courtblockcode"] = linkbtn.CommandArgument.ToString().Trim().Split(',')[2];
            ViewState["courtthanacode"] = linkbtn.CommandArgument.ToString().Trim().Split(',')[3];
            ViewState["courtcourtId"] = linkbtn.CommandArgument.ToString().Trim().Split(',')[4];


            SqlParameter QueryType = new SqlParameter("@QueryType", Convert.ToInt32("9"));

            SqlParameter _DistCode = new SqlParameter("@DISTRICTCODE", ViewState["courtdiscode"].ToString());
            SqlParameter _SubDivision = new SqlParameter("@SubDivisionCode", ViewState["courtsubcode"].ToString());
            SqlParameter _Blockcode = new SqlParameter("@BlockCode", ViewState["courtblockcode"].ToString());
            SqlParameter _pscode = new SqlParameter("@ThanaCode", ViewState["courtthanacode"].ToString());
            SqlParameter _court_id = new SqlParameter("@courtID", ViewState["courtcourtId"].ToString());
            DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _DistCode, _SubDivision, _Blockcode, _pscode, _court_id });
            if (dt.Rows.Count > 0)
            {
                grvnyaayaalay.Columns[1].FooterText = "Total :";
                grvnyaayaalay.Columns[2].FooterText = dt.AsEnumerable().Select(x => x.Field<int>("total")).Sum().ToString();
                grvnyaayaalay.DataSource = dt;
                grvnyaayaalay.DataBind();
            }
            else
            {
                grvnyaayaalay.DataSource = null;
                grvnyaayaalay.DataBind();
            }
            Panelnyaayaalay.Visible = true;
            pnlgrid.Visible = false;
            btnBack.Visible = true;
            ViewState["NayalayType"] = "Rajasav_vavhar_nyaayaalay";
        }
        catch(Exception ex)
        {
    ...
    {
        if((ViewState["NayalayType"].ToString() == "LoK_Uchcha_Sarvochcha") && (Pnlsearch.Visible == true))
        {
            ViewState["NayalayType"] = "";
            Panelnyaayaalay.Visible = false;
            Pnlsearch.Visible = false;
            pnlgrid.Visible = true;
            btnBack.Visible = false;
        }
        else if ((ViewState["NayalayType"].ToString()== "Rajasav_vavhar_nyaayaalay") && (Pnlsearch.Visible == true))
        {
            Panelnyaayaalay.Visible = true;
            Pnlsearch.Visible = false;
            pnlgrid.Visible = false;
    ...
    protected void lnkTotalnayala_Click(object sender, EventArgs e)
    {
        try
        {
            LinkButton lnk = sender as LinkButton;
            string courttype = lnk.CommandArgument.ToString();
            SqlParameter QueryType = new SqlParameter("@QueryType", Convert.ToInt32("10"));
            ...
            SqlParameter _pscode = new SqlParameter("@ThanaCode", ViewState["courtthanacode"].ToString());
            SqlParameter _court_id = new SqlParameter("@courtID", ViewState["courtcourtId"].ToString());
            SqlParameter _courtTypeID = new SqlParameter("@courtTypeID", courttype);
            DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _DistCode, _SubDivision, _Blockcode, _pscode, _court_id, _courtTypeID });
            if (dt.Rows.Count > 0)
            {
    ...
    protected void lnkLUHTOtal_Click(object sender, EventArgs e)
    {   
        try
        {
            LinkButton linkbtn = sender as LinkButton;
            ...
            SqlParameter _pscode = new SqlParameter("@ThanaCode", ViewState["courtthanacode"].ToString());
            SqlParameter _court_id = new SqlParameter("@courtID", ViewState["courtcourtId"].ToString());
            SqlParameter _courtTypeID = new SqlParameter("@courtTypeID", "0");
            DataTable dt = clsData.GetDataTableWithProc("SP_Courtwise", new SqlParameter[] { QueryType, _DistCode, _SubDivision, _Blockcode, _pscode, _court_id, _courtTypeID });
            if (dt.Rows.Count > 0)
            {
            ...
            pnlgrid.Visible = false;
            btnBack.Visible = true;
            ViewState["NayalayType"] = "LoK_Uchcha_Sarvochcha";
        }
        catch (Exception ex)
        {
