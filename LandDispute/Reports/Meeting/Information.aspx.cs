...
public partial class LandDispute_Dashboard_Information : System.Web.UI.Page
{
    ...
    private void ActionDetailsEntryDetails_ka_vivran_old(string a_id, object sender, RepeaterItemEventArgs e)
    {
        SqlParameter _a_id2 = new SqlParameter("@a_id", Convert.ToInt32(a_id));
        SqlParameter _fn2 = new SqlParameter("@fn", "getActionDetailsEntry");

        SqlParameter _fromdate = new SqlParameter("@from_date", Session["from_date"] == null ? null : Convert.ToDateTime(Session["from_date"]).ToString("yyyy-MM-dd"));
        SqlParameter _ToDate = new SqlParameter("@Todate", Session["To_date"] == null ? null : Convert.ToDateTime(Session["To_date"]).ToString("yyyy-MM-dd"));

        DataTable dt_nyayalay_vivran = clsData.GetDataTableWithProc("getEntryPage_print_Information", new SqlParameter[] { _a_id2, _fn2, _fromdate, _ToDate });
        ViewState["gridTable"] = dt_nyayalay_vivran;
