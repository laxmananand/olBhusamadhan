...
public partial class LandDispute_Report_ApplicationConsolidateBlockRpt : System.Web.UI.Page
{
    ...
    //            griddata.Columns[7].FooterText = dt.AsEnumerable().Select(x => x.Field<int>("Prakriyadhin")).Sum().ToString();
    //            griddata.Columns[8].FooterText = dt.AsEnumerable().Select(x => x.Field<int>("Ashwikrit")).Sum().ToString();
    //            griddata.Columns[9].FooterText = dt.AsEnumerable().Select(x => x.Field<int>("Mapi_Nirdharit")).Sum().ToString();
    ...
    //        DataTable dt = clsData.GetDataTableWithProc("Sp_GetDisApplicationConsolidateRpt", new SqlParameter[] { GetQuery, GetDistrictCode, GetBlockCode, GetThanCode, GetPanchayatCode, GetVillageCode, GetWardCode });
